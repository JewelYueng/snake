...
define(['./EventEmitter'], function(EventEmitter){
  ...
  var complieRules = function complieRules(rules_list){
    ...
    var rules_list = rules_list.map(parseRule);
  ...
  .method('start', function(state){
    ...
    this.__states_list__ = complied.states_list;

    if (this.__states_list__.indexOf(state) < 0)
      throw 'Error: state not found!';

    this.state = state;
    this.emit('start', state);
    this.emit(state+'_enter');
    return this;
  ...
  .method('input', function(input){
    ...
    if (!!this.__rules__[input] && !!this.__rules__[input][this.state]){
      ...
      this.emit(this.state+'_leave', input);
      this.emit(this.state+'_to_'+next_state, input);
      this.state = next_state;
      this.emit(next_state+'_enter', input);
      return this;
    }
